...
export const Navbar2 = (props: Navbar2Props) => {
  ...
  return (
    <nav className="flex w-full items-center border-b border-border-secondary bg-green-950 lg:min-h-18 lg:px-[5%]">
      <div className="mx-auto size-full lg:grid lg:grid-cols-[0.375fr_1fr_0.375fr] lg:items-center lg:justify-between lg:gap-4">
        <div className="flex min-h-16 items-center justify-between px-[5%] md:min-h-18 lg:min-h-full lg:px-0">
          <a className="logo rounded-full" href={logo.url}>
        ...
        >
          {navLinks.map((navLink, index) => (
            <div key={index} className="text-white first:pt-4 lg:first:pt-0">
        ...
        <div className="hidden justify-self-end lg:block">
          {buttons.map((button, index) => (
            <Button key={index} className="px-6 py-2 rounded-md text-white" {...button}>
              {button.title}
            </Button>
          ))}
        </div>
      ...
      {isDropdownOpen && (
        <AnimatePresence>
          <motion.nav
            ...
            className="bg-green-950 lg:absolute lg:z-50 lg:border lg:border-border-secondary lg:p-2 lg:[--y-close:25%]"
          >
